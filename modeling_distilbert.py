...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask, head_mask=None, output_attentions=False):
        ...
        mask: torch.tensor(bs, seq_length), 1 for valid, 0 for invalid, modified by ginsoda
...
#         scores.masked_fill_(mask, -float("inf"))  # (bs, n_heads, q_length, k_length)
        ## modified by ginsoda at 2020.10.23 ##
        mask = mask.unsqueeze(1) # (bs, 1, q_length, k_length)
...
class DistilBertModel(DistilBertPreTrainedModel):
    def __init__(self, config):
        ...
        self.embeddings = BertEmbeddings(config)  # Embeddings, modified by ginsoda at 2020.11.10
                                                  # cause distilbert.embedding can't handle token_type ids
    ...
    def forward(
        ...
        ) # (bs, seq_length, dim), modified by ginsoda at 2020.11.18
